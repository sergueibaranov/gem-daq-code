#ifndef gem_supervisor_tbutils_SCurveEvent_h
#define gem_supervisor_tbutils_SCurveEvent_h
...
namespace gem {
  namespace supervisor {
    namespace tbutils {
      ...
      bool keepSCData(string file, int event, const ChannelData& ch){
      ...
      bool keepSCEvent(string file, int event, const VFATEvent& ev, const ChannelData& ch){
        ofstream outf(file.c_str(), ios_base::app );
        if( event<0) return(false);
        if(!outf.is_open()) return(false);
          ...
          keepSCData (file, event, ch);
          outf << hex << ev.crc << dec << endl;
          outf.close();
        return(true);
      };	  

      bool keepSCAppHeader(string file, const AppHeader& ah){
        ofstream outf(file.c_str(), ios_base::app );
        if(!outf.is_open()) return(false);
          outf << ah.minTh << endl;
          outf << ah.maxTh << endl;
