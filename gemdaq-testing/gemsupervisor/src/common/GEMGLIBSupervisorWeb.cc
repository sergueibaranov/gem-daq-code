...
bool gem::supervisor::GEMGLIBSupervisorWeb::selectAction(toolbox::task::WorkLoop *wl)
{
  ...
  if (pDQ) {
    m_counter[0] = *(pDQ+0); // VFAT block counter
    m_counter[1] = *(pDQ+1); // Events counter
    m_counter[2] = *(pDQ+2); // Counter[3]+Counter[4], last eventa
    m_counter[3] = *(pDQ+3); // good VFAT's blocks, last event
    m_counter[4] = *(pDQ+4); // bad VFAT's block, last event 
  }
